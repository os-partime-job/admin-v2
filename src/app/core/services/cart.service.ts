...
export class CartService {
  ...
  removeCartItem(request: any) :Observable<any>{
    return this.http.post<any[]>(`${environment.backEndConfig.apiUrl}/cart/delete`,request,this.httpOptions);
  }
  removeAllCart(){
    this.cartItemList = []
    this.productList.next(this.cartItemList);
  ...
  addProductToCard(request:any): Observable<any>{
    return this.http.post(`${environment.backEndConfig.apiUrl}/cart/add_card`,request,this.httpOptions);
  }
  getProductInCart(request:any): Observable<any>{
    return this.http.post<any[]>(`${environment.backEndConfig.apiUrl}/cart/list`,request,this.httpOptions);
  }
  getProductInCartV2(request:any,token:any): Observable<any>{
    ...
    return this.http.post<any[]>(`${environment.backEndConfig.apiUrl}/cart/list`,request,httpOptions);
  }
  updateProductToCard(request:any): Observable<any>{
    return this.http.post(`${environment.backEndConfig.apiUrl}/cart/update`,request,this.httpOptions);
  }
}
